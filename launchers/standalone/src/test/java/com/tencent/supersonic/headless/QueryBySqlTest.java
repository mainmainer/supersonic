...
public class QueryBySqlTest extends BaseTest {
    ...
    public void testFilterQuery() throws Exception {
        SemanticQueryResp result = queryBySql(
                "SELECT 部门, SUM(访问次数) AS 访问次数 FROM 超音数PVUV统计 WHERE 部门 ='HR' GROUP BY 部门 ", "超音数PVUV统计");
        assertEquals(2, result.getColumns().size());
        QueryColumn firstColumn = result.getColumns().get(0);
        QueryColumn secondColumn = result.getColumns().get(1);
        assertEquals("部门", firstColumn.getName());
    ...
    public void testDateSumQuery() throws Exception {
        ...
        SemanticQueryResp semanticQueryResp = queryBySql(String.format(sql, startDate, endDate), "超音数PVUV统计");
        assertEquals(1, semanticQueryResp.getColumns().size());
        QueryColumn queryColumn = semanticQueryResp.getColumns().get(0);
        assertEquals("访问次数", queryColumn.getName());
        assertEquals(1, semanticQueryResp.getResultList().size());
    ...
    public void testCacheQuery() throws Exception {
        queryBySql("SELECT 部门, SUM(访问次数) AS 访问次数 FROM 超音数PVUV统计  GROUP BY 部门 ", "超音数PVUV统计");
    ...
    public void testBizNameQuery() throws Exception {
        SemanticQueryResp result1 = queryBySql("SELECT SUM(pv) FROM 超音数PVUV统计  WHERE department ='HR'", "超音数PVUV统计");
        SemanticQueryResp result2 = queryBySql("SELECT SUM(访问次数) FROM 超音数PVUV统计  WHERE 部门 ='HR'", "超音数PVUV统计");
    ...
    public void testAuthorization_model() {
        ...
        assertThrows(InvalidPermissionException.class,
                () -> queryBySql("SELECT SUM(pv) FROM 超音数PVUV统计  WHERE department ='HR'", alice, "超音数PVUV统计"));
    }
    ...
    public void testAuthorization_sensitive_metric() throws Exception {
        ...
        assertThrows(InvalidPermissionException.class,
                () -> queryBySql("SELECT SUM(stay_hours) FROM 停留时长统计  WHERE department ='HR'", tom, "停留时长统计"));
    }
    ...
    public void testAuthorization_sensitive_metric_jack() throws Exception {
        ...
        SemanticQueryResp semanticQueryResp =
                queryBySql("SELECT SUM(stay_hours) FROM 停留时长统计", jack, "停留时长统计");
    ...
    public void testAuthorization_row_permission() throws Exception {
        User tom = DataUtils.getUserTom();
        SemanticQueryResp semanticQueryResp =
                queryBySql("SELECT SUM(pv) FROM 超音数PVUV统计  WHERE department ='HR'", tom, "超音数PVUV统计");
